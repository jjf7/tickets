...
componentDidMount() {
    ...
    UserService.getTicket(this.state.id).then(
      response => {
		 
        this.setState({
          ticket: response.data[0].ticket_pedido,user:response.data[0].id_user
        });
      },
      error => {
        this.setState({
  ...
  handleSubmit(e) {
    ...
    if (this.checkBtn.context._errors.length === 0) {
      ...
      UserService.updateTicket(this.state.id,this.state.user, this.state.ticket).then(
        (response) => {
    ...
    return (
      <div className="col-md-4 mx-auto pt-5">
        <div className="card card-container p-2">
          <h4 className="text-center">Editar ticket</h4>
